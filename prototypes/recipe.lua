...
data:extend({
  -- [[
  { -- explosives (item) -- could be autogenerated if not blacklisted
    type = "recipe",
...
-- space fish for bio rockets in space
if settings.startup["space-fish"].value then
  data:extend({
    { -- fish-breeding shamelessly copied from space-age-dlc. space version
      ...
      results = { { type = "item", name = "raw-fish", amount = 4, reset_freshness_on_craft = true } }, -- percent_spoiled=0.5
...
if settings.startup["armour-plating"].value then
  data:extend({
    { -- light armour plating
      ...
      categories = { "advanced-crafting", "crafting-with-fluid" }, --not handcraftable, assembler-2 needed
      group = "combat",
  ...
  if settings.startup["earlier-armor-plating"].value then
    ...
    data.raw["recipe"]["light-armour-plating"].categories = { "crafting", "advanced-crafting", "crafting-with-fluid" } -- assembler-1 allowed
  end
